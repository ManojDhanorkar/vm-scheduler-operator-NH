...
// AWSVMSchedulerStopNHReconciler reconciles a AWSVMSchedulerStopNH object
type AWSVMSchedulerStopNHReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopnhs,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopnhs/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopnhs/finalizers,verbs=update
// +kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStopNH object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *AWSVMSchedulerStopNHReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)
	//log := r.Log.WithValues("AWSVMScheduler", req.NamespacedName)
	log.Info("Reconciling AWSVMSchedulerStopNH")
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStopNH{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStopNH(awsVMScheduler)
...
func (r *AWSVMSchedulerStopNHReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStopNH{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

// CronJob Spec
func (r *AWSVMSchedulerStopNHReconciler) cronJobForAWSVMSchedulerStopNH(awsVMScheduler *awsv1.AWSVMSchedulerStopNH) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStopNHLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
// // Check if the cronJob is created, if not create onekubectl get all -n vm-scheduler-operator-system
...
func AWSVMSchedulerStopNHLabels(v *awsv1.AWSVMSchedulerStopNH, tier string) map[string]string {
	return map[string]string{
		"app":                     "AWSVMSchedulerStopNH",
		"AWSVMSchedulerStopNH_cr": v.Name,
		"tier":                    tier,
	}
}
