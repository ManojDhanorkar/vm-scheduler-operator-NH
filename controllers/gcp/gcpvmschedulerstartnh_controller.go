...
// GCPVMSchedulerStartNHReconciler reconciles a GCPVMSchedulerStartNH object
type GCPVMSchedulerStartNHReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=gcp.xyzcompany.com,resources=gcpvmschedulerstartnhs,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=gcp.xyzcompany.com,resources=gcpvmschedulerstartnhs/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=gcp.xyzcompany.com,resources=gcpvmschedulerstartnhs/finalizers,verbs=update
// +kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the GCPVMSchedulerStartNH object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *GCPVMSchedulerStartNHReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)

	log.Info("Reconciling GCPVMSchedulerStartNH")
	...
	gcpVMScheduler := &gcpv1.GCPVMSchedulerStartNH{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForGCPVMSchedulerStartNH(gcpVMScheduler)
	...
	newStartSchedule := gcpVMScheduler.Spec.StartSchedule
...
func (r *GCPVMSchedulerStartNHReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&gcpv1.GCPVMSchedulerStartNH{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

func (r *GCPVMSchedulerStartNHReconciler) cronJobForGCPVMSchedulerStartNH(gcpVMScheduler *gcpv1.GCPVMSchedulerStartNH) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      gcpVMScheduler.Name,
			Namespace: gcpVMScheduler.Namespace,
			Labels:    GCPVMSchedulerStartNHLabels(gcpVMScheduler, "gcpVMScheduler"),
		},
...
func GCPVMSchedulerStartNHLabels(v *gcpv1.GCPVMSchedulerStartNH, tier string) map[string]string {
	return map[string]string{
		"app":                      "GCPVMSchedulerStartNH",
		"GCPVMSchedulerStartNH_cr": v.Name,
		"tier":                     tier,
	}
}
